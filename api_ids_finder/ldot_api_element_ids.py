...
def get_deelnemer_entity_id(study_id: str):
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to get entity IDs, status code: {response.status_code}, message: {response.text}"
        )

    else:
        entities = response.json().get("Data", {}).get("StudyEntities", [])
        entity_ids = {
            entity["Description"]: entity["Guid"]
            for entity in entities
            if entity["Description"] == "Deelnemer"
        }
...
def get_eaids_of_all_event_actions(study_id: str):
    ...
    if response.status_code != 200:
        raise Exception(
            f"Failed to get event action IDs, status code: {response.status_code}, message: {response.text}"
        )

    else:
        actions_list = response.json().get("Data", {}).get("StudyEventActions", [])
        actions_list = {
            action["Description"]: action["EventActionGuid"] for action in actions_list
        }
        return actions_list
