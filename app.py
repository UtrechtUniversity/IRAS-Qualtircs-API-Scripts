...
def get_clients_for_study(study_variables: StudySettings):
    if not study_variables.config_path:
        raise ValueError(
            f"Missing config_path for study_id: {study_variables.study_id}"
        )
    ...
    with CLIENT_CACHE_LOCK:
        ...
        ldot_client_secret = study_env.get("LDOT_client_secret") or os.environ.get(
            "LDOT_client_secret"
        )
        qualtrics_api_token = study_env.get("QUALTRICS_API_TOKEN") or os.environ.get(
            "QUALTRICS_API_TOKEN"
        )
        ...
        if missing_secrets:
            raise KeyError(
                f"Missing secrets in {study_env_path}: {', '.join(missing_secrets)}"
            )
...
def study_work_units(study_id):
    ...
    if not study_variables:
        return jsonify(
            {"success": False, "message": f"Unknown study_id: {study_id}"}
        ), 404
