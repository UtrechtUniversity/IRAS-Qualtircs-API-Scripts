...
def check_contact_in_mailing_list(
    qualtrics_client, study_identifier: str, mailing_list_id: str, directory_id: str
) -> bool:
...
def add_contact_to_mailing_list(
    qualtrics_client,
...
def get_personal_link(
    ...
    try:
        ...
        personal_link = [
            ...
            if ("externalDataReference" in item)
            and (item["externalDataReference"] == study_identifier)
        ][0]
...
def add_individuals_to_survey(
    ldot_client,
    qualtrics_client,
    new_subject_ids: list,
    ldot_study_id: str,
    ...
    for subject_id in new_subject_ids:
        ...
        contact_exists = check_contact_in_mailing_list(
            qualtrics_client, study_identifier, mailing_list_id, directory_id
        )
        ...
        personal_link = get_personal_link(
            qualtrics_client, qualtrics_survey_id, distribution_id, study_identifier
        )
