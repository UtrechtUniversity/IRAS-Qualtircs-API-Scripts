...
class QualtricsExportService:
    ...
    def _get_responses_as_df(self,incomplete_bool: bool) -> pd.DataFrame:
        ...
        while export_progress < 100:
            export_progress = self._check_export_progress(request_id, returns="percent_complete")
            sleep(2)
...
class CheckSurveyProgressWorkflow:
    ...
    def run(self, trigger, resolution, qualtrics_survey_id, embedded_data_field):
        ...
        subject_id_to_study_identifier_dict = self._subject_id_to_study_identifier(ready_subject_ids)
        subject_id_to_progress_dict = {}

        qualtrics_export_service = QualtricsExportService(self.qualtrics_client, qualtrics_survey_id)
        responses_df = qualtrics_export_service.get_full_responses()

        if not embedded_data_field in responses_df.columns:
            raise ValueError(f"Embedded data field '{embedded_data_field}' not found in survey data. Please check the field name.")


        for subject_id, study_identifier in subject_id_to_study_identifier_dict.items():
            if study_identifier is None:
                subject_id_to_progress_dict[subject_id] = 0  # Indicator that the subject ID was not found in the data
                continue
            individual_progress = responses_df[responses_df[embedded_data_field] == study_identifier]
            if not individual_progress.empty:
                ...
                progress_percentage = individual_progress["Progress"].astype(float).min()
    ...
    def _subject_id_to_study_identifier(self, ready_subject_ids: list) -> dict:
        ...
        for subject_id in ready_subject_ids:
            ...
            subject_id_to_study_identifier_dict[subject_id] = response.json().get("Data", {}).get("Subject", {}).get("RegistrationId", None)

        return subject_id_to_study_identifier_dict

    def _get_ready_subjects(self, eaid_qualtrics_survey_progress_to_do_date: str) -> list:
        ...
        try:
            ...
            raise ValueError("Ldot lookup to find subjects with Qualtrics survey progress to do date returned invalid JSON", response) from e
    ...
    def _send_progress_to_ldot(self, subject_id_to_progress_dict: dict, eaid_survey_progress_completed: str) -> list:
...
def handle_check_qualtrics_survey_module(ldot_client, qualtrics_client, study_variables, unit):
    ...
    workflow = CheckSurveyProgressWorkflow(ldot_client, qualtrics_client, ldot_study_id, id_deelnemer_entity, id_location)
